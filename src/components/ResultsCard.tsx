...
interface Field {
  ...
  value: unknown;
  warn?: boolean;
  aiGen?: boolean;  // true if this value was AI-generated (show * marker)
}
...
function FieldBlock({ field }: { field: Field }) {
  ...
  return (
    ...
    >
      <div
        className="font-ui uppercase text-[8.5px] font-bold tracking-widest-2 mb-1.5 flex items-center gap-1.5"
        style={{ color: "var(--text-muted)" }}
      >
        {field.label}
        {field.aiGen && !missing && (
          <span
            title="Generated by AI — may not be fully accurate. Verify on the official website."
            style={{ color: "var(--warning)", fontSize: "10px", lineHeight: 1 }}
          >
            *
          </span>
        )}
...
export function ResultsCard({ data }: { data: ScrapeRecord }) {
  ...
  // Helper: check if a field (or nested sub-field) was AI-generated
  const ai = data.ai_generated_fields ?? {};
  const isAI = (field: string) => !!ai[field];
    ...
    () => [
      {
        ...
        fields: [
          { label: "Intake months", value: data.intake_months, aiGen: isAI("intake_months") },
          { label: "Application deadlines", value: data.application_deadlines, aiGen: isAI("application_deadlines") },
        ],
      },
      {
        title: "Academic Requirements",
        fields: [
          { label: "Min. academic requirement", value: data.min_academic_requirement, aiGen: isAI("min_academic_requirement") },
          { label: "Accepted qualifications", value: data.accepted_qualifications, aiGen: isAI("accepted_qualifications") },
          { label: "Work experience", value: data.work_experience, aiGen: isAI("work_experience") },
        ],
      },
      {
        title: "Language Requirements",
        fields: [
          { label: "IELTS", value: eng.ielts, aiGen: isAI("english_requirements.ielts") },
          { label: "TOEFL", value: eng.toefl, aiGen: isAI("english_requirements.toefl") },
          { label: "PTE", value: eng.pte, aiGen: isAI("english_requirements.pte") },
          { label: "Duolingo", value: eng.duolingo, aiGen: isAI("english_requirements.duolingo") },
      ...
      },
      {
        ...
        fields: [
          { label: "Other requirements", value: data.other_requirements, aiGen: isAI("other_requirements") },
  ...
  const hasAiFields = Object.keys(ai).length > 0;
  ...
  return (
    ...
    >
      ...
      {/* AI disclaimer — shown only when AI-generated fields are present */}
      {hasAiFields && (
        <div
          className="px-8 py-3 flex items-start gap-2"
          style={{ borderTop: "1px solid rgba(226, 175, 112, 0.12)", background: "rgba(226, 175, 112, 0.03)" }}
        >
          <span style={{ color: "var(--warning)", fontSize: "11px", lineHeight: 1, marginTop: "2px" }}>*</span>
          <p
            className="font-ui text-[10px] leading-relaxed"
            style={{ color: "var(--text-muted)" }}
          >
            Fields marked with an asterisk were generated by AI using secondary web sources and may not be fully accurate.
            Always verify important details on the{" "}
            <a
              href={data.url_requested}
              target="_blank"
              rel="noreferrer"
